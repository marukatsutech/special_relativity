...
class ArrowPrecession:
    def __init__(self, ax, x, y, z, is_normalize, length, theta_axis, phi_axis, theta_precession, phase_precession,
                 color_axis, color_arrow, line_width, line_style, label_axis, label_arrow):
        self.ax = ax
        self.x, self.y, self.z = x, y, z
        self.is_normalize = is_normalize
        self.r = length
        ...
        self.theta_precession = theta_precession
        self.phase_precession = phase_precession
        self.color_axis = color_axis
        self.color_arrow = color_arrow
        self.line_width = line_width
        self.line_style = line_style
        self.label_axis = label_axis
        self.label_arrow = label_arrow

        self.u, self.v, self.w = spherical_to_cartesian(self.r, self.theta_axis_init, self.phi_axis_init)

        self.arrow_axis = Arrow3d(self.ax, 0., 0., 0., True, 1., 0., 0.,
                                  self.color_axis, self.line_width, self.line_style, self.label_axis)

        self.r_precession = np.sin(self.theta_precession)
        self.circle_precession = Circle3d(self.ax, 0., 0., np.cos(self.theta_precession), self.r_precession,
                                          "z", self.line_width, self.line_style, self.color_arrow, 1)

        self.point_precession = np.array([self.r_precession * np.cos(self.phase_precession),
                                          self.r_precession * np.sin(self.phase_precession),
                                          np.cos(self.theta_precession)])
        self.light_arrow = Arrow3d(self.ax, 0., 0., 0., False, 1., 0., 0.,
                                   self.color_arrow, self.line_width, self.line_style, self.label_arrow)
        self.light_arrow.set_head_point(self.point_precession[0], self.point_precession[1], self.point_precession[2])
    ...
    def set_direction(self, theta, phi):
        self.u, self.v, self.w = spherical_to_cartesian(self.r, theta, phi)
        self.arrow_axis.set_direction(theta, phi)
        self.circle_precession.set_direction(theta, phi)

        rot_matrix_theta = Rotation.from_rotvec(theta * vector_y_axis)
        rot_matrix_phi = Rotation.from_rotvec(phi * vector_z_axis)
        vector_rotated_theta = rot_matrix_theta.apply(self.point_precession)
        vector_rotated = rot_matrix_phi.apply(vector_rotated_theta)
        self.light_arrow.set_head_point(vector_rotated[0], vector_rotated[1], vector_rotated[2])

    def set_precession_phase(self, phase):
        self.phase_precession = phase
        self.point_precession = np.array([self.r_precession * np.cos(self.phase_precession),
                                          self.r_precession * np.sin(self.phase_precession),
                                          np.cos(self.theta_precession)])
        self.light_arrow.set_head_point(self.point_precession[0], self.point_precession[1], self.point_precession[2])

    def get_arrow_vector(self):
        return self.light_arrow.get_vector()
...
def update_diagrams():
    theta_precession = np.deg2rad(cnt.get() % 360) * 10.

    theta = np.deg2rad(np.random.rand() * 180.)
    phi = np.deg2rad(np.random.rand() * 360.)

    light_arrow_precession.set_precession_phase(theta_precession)
    light_arrow_precession.set_direction(theta, phi)

    vector_light_arrow = light_arrow_precession.get_arrow_vector()
    vector_light_arrow = vector_light_arrow / np.linalg.norm(vector_light_arrow)
    r, theta_light_vib, phi_light_vib = cartesian_to_spherical(
        vector_light_arrow[0], vector_light_arrow[1], vector_light_arrow[2])

    light_vibration_circle.set_direction(theta_light_vib, phi_light_vib)
    light_vibration_line.set_direction(nu_light * theta_light_vib, theta_light_vib, phi_light_vib)

    for i in range(30):
        theta = np.deg2rad(np.random.rand() * 180.)
        phi = np.deg2rad(np.random.rand() * 360.)
        light_arrow_afterimages[i].set_precession_phase(theta_precession)
...
if __name__ == "__main__":
    ...
    # light_arrow = Arrow3d(ax0, 0., 0., 0., True,1., 0., 0., "darkorange", 2, "-", "Light-arrow")

    light_arrow_precession = ArrowPrecession(ax0, 0., 0., 0., True, 1., 0.,
                                            0., np.deg2rad(10.), 0.,
                                             "blue", "darkorange", 1.5,
                                             "-", "Rotating vector", "Light-arrow")
    ...
    for i in range(30):
        theta = np.deg2rad(np.random.rand() * 180.)
        phi = np.deg2rad(np.random.rand() * 360.)
        light_arrow_afterimage = ArrowPrecession(ax0, 0., 0., 0., True, 1., 0.,
                                                 0., np.deg2rad(10.), 0.,
                                                 "gray", "gray", 0.5,
                                                 "--", "", "")
