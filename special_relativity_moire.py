# Special relativity and moire pattern
...
def draw_parallel_lines(th, x_center, y_center, width, height, pth, col, size):
    ...
    for i in range(len(lines_start)):
        ...
        ax2.plot([p_start_rot[0] + x_center, p_end_rot[0] + x_center],
...
def draw_hyperbolic_rot(th):
    ...
    hyperbolic2_rotated = np.dot(rot, hyperbolic2_rot)
...
def draw_hyperbolic():
    # Hyperbolic curve; ct**2 - x**2 = 1 (y**2 - x**2 = 1)
    y = np.sqrt(1. + x ** 2.)
    ax1.plot(x, y, linewidth=1, c='magenta', label='ct**2 - x**2 = 1 (B)', linestyle='--')
    y = - np.sqrt(1. + x ** 2.)
    ax1.plot(x, y, linewidth=1, c='magenta', linestyle='--')
...
def draw_common():
    ...
    # Concentric circles
    draw_circles(0., 0., 0., 6., 6., pitch_circles, 'darkorange', 2)
    # Show pitch
    ptc_l = pitch_parallel * pitch_percent / 100.
    ratio = ptc_l / pitch_circles
    ax2.text(x_min, y_max * 0.95, "Pitch of circles=" + str(pitch_circles))
    ax2.text(x_min, y_max * 0.90, "Pitch of lines=" + str(f'{ptc_l:.3f}'))
    ax2.text(x_min, y_max * 0.85, "Ratio of pitch(lines/circles)=" + str(f'{ratio:.3f}'))
...
# Hyperbolic curve; ct**2 - x**2 = 1 (y**2 - x**2 = 1) for rotation
x_rot = np.linspace(x_min, x_max, num_of_points)
y1_rot = np.sqrt(1. + x ** 2.)
y2_rot = - np.sqrt(1. + x ** 2.)
