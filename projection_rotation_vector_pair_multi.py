""" Projection of rotation vector pair (multi pairs) """
import numpy as np
...
class InternalPhase:
    ...
    def _rotate_all(self, angle, center_axis):
        ...
        self.vector_tilt_center_axis = rot_matrix.apply(self.vector_tilt_center_axis)
    ...
    def set_rotate_offset_z(self, angle):
        self._rotate_all(angle, vector_z_axis)
...
def switch_tilt():
    ...
    # 2
    internal_phase_2.hide_rotation_vector_time()
    projection_guide_2.reset()
    projection_guide_2.offset(0., 0., 1.)
    # 3
    internal_phase_3.hide_rotation_vector_time()
    projection_guide_3.reset()
    projection_guide_3.offset(0., 0., 1.)
    # 3
    internal_phase_4.hide_rotation_vector_time()
    projection_guide_4.reset()
    projection_guide_4.offset(0., 0., 1.)
    if is_tilt:
        pass
    else:
        # 1
        ...
        # 2
        internal_phase_2.tilt(np.deg2rad(45))
        internal_phase_2.show_rotation_vector_time()
        projection_guide_2.offset(0., -1., 0.)
        projection_guide_2.rotate(np.deg2rad(45), vector_z_axis)
        # 3
        internal_phase_3.tilt(np.deg2rad(45))
        internal_phase_3.show_rotation_vector_time()
        projection_guide_3.offset(0., -1., 0.)
        projection_guide_3.rotate(np.deg2rad(90), vector_z_axis)
        # 4
        internal_phase_4.tilt(np.deg2rad(45))
        internal_phase_4.show_rotation_vector_time()
        projection_guide_4.offset(0., -1., 0.)
        projection_guide_4.rotate(np.deg2rad(135), vector_z_axis)

    is_tilt = not is_tilt

    update_intersection()
    update_intersection_2()
    update_intersection_3()
    update_intersection_4()


def clear_path():
    projection_path.clear_path()
    projection_path_2.clear_path()
    projection_path_3.clear_path()
    projection_path_4.clear_path()
...
def update_intersection_2():
    # Check intersection
    line1 = (np.array([0, 0, 0]), internal_phase_2.get_rotation_vector_spatial())
    line2 = (projection_guide_2.get_start(), projection_guide_2.get_end() - projection_guide_2.get_start())

    intersection = find_intersection(line1, line2)

    if intersection is not None:
        projection_line_2.set_start_end(0., 0., 0,
                                        intersection[0], intersection[1], intersection[2])
        return intersection[0], intersection[1], intersection[2]
    else:
        print("No intersection exists.")
...
def update_intersection_3():
    # Check intersection
    line1 = (np.array([0, 0, 0]), internal_phase_3.get_rotation_vector_spatial())
    line2 = (projection_guide_3.get_start(), projection_guide_3.get_end() - projection_guide_3.get_start())

    intersection = find_intersection(line1, line2)

    if intersection is not None:
        projection_line_3.set_start_end(0., 0., 0,
                                        intersection[0], intersection[1], intersection[2])
        return intersection[0], intersection[1], intersection[2]
    else:
        print("No intersection exists.")
...
def update_intersection_4():
    # Check intersection
    line1 = (np.array([0, 0, 0]), internal_phase_4.get_rotation_vector_spatial())
    line2 = (projection_guide_4.get_start(), projection_guide_4.get_end() - projection_guide_4.get_start())

    intersection = find_intersection(line1, line2)

    if intersection is not None:
        projection_line_4.set_start_end(0., 0., 0,
                                        intersection[0], intersection[1], intersection[2])
        return intersection[0], intersection[1], intersection[2]
    else:
        print("No intersection exists.")
...
def update_diagrams():
    ...
    # 2
    internal_phase_2.rotate_phase()
    # 3
    internal_phase_3.rotate_phase()
    # 3
    internal_phase_4.rotate_phase()
    if is_tilt:
        # 1
        internal_phase.rotate_z()
        projection_guide.rotate(np.deg2rad(phase_step_deg), vector_z_axis)
        # 2
        internal_phase_2.rotate_z()
        projection_guide_2.rotate(np.deg2rad(phase_step_deg), vector_z_axis)
        # 3
        internal_phase_3.rotate_z()
        projection_guide_3.rotate(np.deg2rad(phase_step_deg), vector_z_axis)
        # 4
        internal_phase_4.rotate_z()
        projection_guide_4.rotate(np.deg2rad(phase_step_deg), vector_z_axis)

    # 1
    x, y, z = update_intersection()
    projection_path.update_path(x, y, z)
    # 2
    x2, y2, z2 = update_intersection_2()
    projection_path_2.update_path(x2, y2, z2)
    # 3
    x3, y3, z3 = update_intersection_3()
    projection_path_3.update_path(x3, y3, z3)
    # 4
    x4, y4, z4 = update_intersection_4()
    projection_path_4.update_path(x4, y4, z4)
...
def reset():
    ...
    update_intersection()
    # 2
    internal_phase_2.reset()
    internal_phase_2.set_rotate_offset_z(np.deg2rad(45))
    projection_guide_2.reset()
    projection_guide_2.offset(0., 0., 1.)
    projection_guide_2.rotate(np.deg2rad(45), vector_z_axis)
    update_intersection_2()
    # 3
    internal_phase_3.reset()
    internal_phase_3.set_rotate_offset_z(np.deg2rad(90))
    projection_guide_3.reset()
    projection_guide_3.offset(0., 0., 1.)
    projection_guide_3.rotate(np.deg2rad(90), vector_z_axis)
    update_intersection_3()
    # 4
    internal_phase_4.reset()
    internal_phase_4.set_rotate_offset_z(np.deg2rad(135))
    projection_guide_4.reset()
    projection_guide_4.offset(0., 0., 1.)
    projection_guide_4.rotate(np.deg2rad(135), vector_z_axis)
    update_intersection_4()
...
if __name__ == "__main__":
    ...
    # 2
    internal_phase_2 = InternalPhase(ax0, np.deg2rad(phase_step_deg))
    internal_phase_2.set_rotate_offset_z(np.deg2rad(45))
    internal_phase_2.hide_rotation_vector_time()
    projection_guide_2 = Line3D(ax0, -8., 8., 0., 0., 0., 0.,
                                0, 0, 1, "--", "green", 1)
    projection_guide_2.offset(0., 0., 1.)
    projection_guide_2.rotate(np.deg2rad(45), vector_z_axis)
    projection_line_2 = Line3D(ax0, 0., 0., 0., 0., 0., 0.,
                               0, 3, 1, "--", "darkorange", 1)

    update_intersection_2()

    projection_path_2 = ProjectionPath(ax0, 1, "-", "darkorange", 1)

    # 3
    internal_phase_3 = InternalPhase(ax0, np.deg2rad(phase_step_deg))
    internal_phase_3.set_rotate_offset_z(np.deg2rad(90))
    internal_phase_3.hide_rotation_vector_time()
    projection_guide_3 = Line3D(ax0, -8., 8., 0., 0., 0., 0.,
                                0, 0, 1, "--", "green", 1)
    projection_guide_3.offset(0., 0., 1.)
    projection_guide_3.rotate(np.deg2rad(90), vector_z_axis)
    projection_line_3 = Line3D(ax0, 0., 0., 0., 0., 0., 0.,
                               0, 3, 1, "--", "darkorange", 1)

    update_intersection_3()

    projection_path_3 = ProjectionPath(ax0, 1, "-", "darkorange", 1)

    # 3
    internal_phase_4 = InternalPhase(ax0, np.deg2rad(phase_step_deg))
    internal_phase_4.set_rotate_offset_z(np.deg2rad(135))
    internal_phase_4.hide_rotation_vector_time()
    projection_guide_4 = Line3D(ax0, -8., 8., 0., 0., 0., 0.,
                                0, 0, 1, "--", "green", 1)
    projection_guide_4.offset(0., 0., 1.)
    projection_guide_4.rotate(np.deg2rad(135), vector_z_axis)
    projection_line_4 = Line3D(ax0, 0., 0., 0., 0., 0., 0.,
                               0, 3, 1, "--", "darkorange", 1)

    update_intersection_4()

    projection_path_4 = ProjectionPath(ax0, 1, "-", "darkorange", 1)
